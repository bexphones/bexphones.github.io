f.start_game = function () {
	...
	var ev = { //events
		t: [t.searching_opponent * (o.searching_opponent.number + 1) * 2, t.cloud + o.cloud_tw[0].d, 900, 8000]
		//all events are sum to the previous
	};ev.tc = [];
	var summed_actions = 0;
	f.convert_events_time = function () {
	...
	f.start_cloud = function () {
		...
		var tw_appears_puissance = {
			o: interf.puissance[0],
			t: 500,
			d: 0,
			e: Phaser.Easing.Exponential.Out,
			sy: 1,
		...
		var tw_appears_progress_bg = {
			o: interf.progress[0].bg,
			t: 500,
			d: 0,
			e: Phaser.Easing.Exponential.Out,
			sy: 1,
		...
		var tw_appears_progress_main = {
			o: interf.progress[0].main,
			t: 500,
			d: 0,
			e: Phaser.Easing.Exponential.Out,
			sy: 1,
		...
		wait(function () {
			interf[0].visible = true;
		}, o.cloud_tw[0].d);
		wait(function () {
			_tr(tw_appears_enemy);
		}, o.cloud_tw[0].d);

		wait(function () {
			interf.roll[0].visible = true;
		}, o.cloud_tw[0].d);
		wait(function () {
			_tr(tw_appears_roll);
		}, o.cloud_tw[0].d);
		wait(function () {
			interf.points[0].visible = true;
		}, o.cloud_tw[0].d);
		wait(function () {
			_tr(tw_appears_points);
		}, o.cloud_tw[0].d);

		wait(function () {
			interf.puissance[0].visible = true;
		}, o.cloud_tw[0].d);
		wait(function () {
			_tr(tw_appears_puissance);
		}, o.cloud_tw[0].d);

		wait(function () {
			interf.progress[0].visible = true;
		}, o.cloud_tw[0].d);
		//progress bar de l'enemy
		wait(function () {
			_tr(tw_appears_progress_bg);
		}, o.cloud_tw[0].d);
		wait(function () {
			_tr(tw_appears_progress_main);
		}, o.cloud_tw[0].d);
	};

	// compte à rebours pour lancer le jeu
	f.start_timer = function () {
		interf.decount.visible = true;
		var ready = function ready() {
			interf.decount.text = "ready";
			o.ready_tw = {
				o: interf.decount, //object
				t: 900, //time
				d: 100, //delay
				//a: 1, //alpha
		...
		//wait(decount,1400)
		wait(ready, 100);
		//wait(()=>{interf.decount.visible=false},800)
	};
	...
	wait(function () {
		interf.progress[0].main.visible = true;
	}, ev.tc[0]);
	wait(function () {
		interf.progress[0].bg.visible = true;
	}, ev.tc[0]);
	wait(f.start_timer, ev.tc[1]);

	wait(function () {
		f.wait_start_game(o.paper[0], 0);
	}, ev.tc[2]);
	wait(function () {
		f.wait_start_game(o.paper[1], 0);
	}, ev.tc[2]);
